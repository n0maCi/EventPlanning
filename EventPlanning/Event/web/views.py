    ...
    if request.POST:
        if 'add' in request.POST:
            ...
            if request.method == 'POST':
                ...
                edit_db = Events.objects.all().filter(title=data).values_list("id")
                id_input = UserHasEvent.objects.filter(title_events=data).values_list("id")
                try:
...
@login_required
def add_event_view(request, title = None, name = None):
    owner_id = UserHasEvent.objects.all().filter(title_events=title).values_list("id")[0][0]
    ...
    try:
        ...
        Events.objects.filter(id=edit_db.values_list("id")[0][0]).delete()
    except IndexError:
        print('Лох')

    records = Events.objects.all().filter(title=title).order_by('start_at')
    return render(request, 'web/add.html', {'records': records, 'title': title})


def event_view(request, title):
    records = Events.objects.all().filter(title=title).order_by('start_at')
